...
- ✅ TLS (2.7) и Mongo auth (2.6) — включены 16.09.2026 (`1f50d4e`); см. разделы 2.6/2.7.
...
- ✅ **Статус: ✅ FIXED (`1f50d4e`).** `mongod --auth`; бэкенд и экспортёр ходят под `anyq_app` с `readWrite` на одну базу, root-пользователь только для администрирования. Переход на живом стеке — `scripts/mongo_enable_auth.sh`. Проверено: без пароля `countDocuments` отказывает, app-пользователь не видит чужих баз.

### 2.7 Нет TLS — всё в открытом виде
- ✅ **Статус: ✅ FIXED (`1f50d4e`).** `HTTPS_TERMINATED=1` — nginx слушает 3443 (TLS 1.2/1.3), порт 3000 отвечает 301; без сертификата контейнер не стартует. Кука получает `Secure` от того же переключателя. Проверено: TLSv1.3, Secure-кука, wss ping/chat.
...
| 3.7 | Ошибки загрузки чатов глотаются; loading/error dead code | ✅ FIXED после слияния фронта: кадры WS разбираются поле за полем (`str()` в `lib/live.tsx`), приведения `data as ...` на границе нет |
...
| 3.12 | Mongo — см. 2.6 | ✅ FIXED (см. 2.6) |
...
1. `_latex_is_available()` не вызывается — ✅ FIXED 17.09.2026 (удалена).
2. `_heuristic_video_needed` недостижимая ветка — ✅ FIXED 17.09.2026 (ветка удалена, тип возврата без `Optional`).
3. `format_output` возвращает `None` при video_needed=false (агент жмёт `or ""`) — ✅ FIXED 17.09.2026 (возвращает `""`, как `reject_non_science`).
...
## 5a. Дыры, найденные при повторной проверке (17.09.2026)

| Что было | Последствие | Статус |
|---|---|---|
| IP для лимитов брался из **первой** записи `X-Forwarded-For`, а nginx дописывает настоящий адрес в **конец** | Придуманный адрес в заголовке = свежий бюджет на каждый запрос; лимиты входа и регистрации не держали никого | ✅ FIXED: `X-Real-IP` (его nginx перезаписывает), иначе последняя запись |
| Успешный вход обнулял счётчик адреса | Со своим аккаунтом можно стирать счётчик между попытками подобрать чужие пароли | ✅ FIXED: адрес тратят только ошибки (`LOGIN_IP_MAX_FAILURES=50`), успех его не трогает — класс за одним NAT не блокируется успешными входами |
| Ключ аккаунта в лимите — email как напечатан | Каждый вариант регистра букв — отдельные 5 попыток | ✅ FIXED: ключ в нижнем регистре, как и поиск |
| Удаление аккаунта не трогало `assessments`, `lesson_plans`, `saved_explanations` | Документы удалённого аккаунта навсегда оставались в базе | ✅ FIXED |
| `PUT /api/lesson-plans/{id}` сохранял любой JSON | Мегабайты в одном плане, чужие поля (имя педагога, дата), этап-число, который страница не может нарисовать | ✅ FIXED: только поля документа, строки, ≤30 этапов, ≤4000 символов на поле; формулировки не трогаются |
| У `lesson_plans` не было индекса | Список планов — полный скан коллекции | ✅ FIXED |
| `PENDING_REQUESTS_TTL_SEC` не пробрасывался в compose | Настройку нельзя было поменять из `.env` | ✅ FIXED |
...
5. ✅ TLS (2.7) и Mongo auth (2.6) — включены (`1f50d4e`).
